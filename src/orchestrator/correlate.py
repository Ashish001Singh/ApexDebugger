import re
from src.review_core.models import Finding, RuleId, Severity,ReviewResult
from pathlib import Path

_APEX_IMPORT = re.compile(r"@salesforce/apex/(\w+)\.\w+")
_SECURITY_RULES = {"missing_crud_fls", "missing_sharing_declaration"}

def correlate(results, lwc_sources) -> list[ReviewResult]:
    insecure = {Path(r.filename).stem for r in results
        ...
        for controller in set(_APEX_IMPORT.findall(js)):
            if controller in insecure:
                ...
                out.append(ReviewResult(
                    filename=f"(cross-language) · {controller}",   # ← controller in the identity
                    findings=[finding],
                ))
    return out